    ...
    for (id, mut state, confirm, confirm_state) in query.iter_mut() {
        match *state {
            ...
            ref asky_state => {
                eprint!(".");
                let highlight =
                    TextStyle { color: if matches!(asky_state, AskyState::Reading) {
                ...
                if matches!(asky_state, AskyState::Complete) {
                    bundles.push(TextBundle::from_section(match confirm_state.yes {
                        Some(true) => " Yes",
                        Some(false) => " No",
                        None => unreachable!(),
                    }, TextStyle {
                    ...
                    bundles.push(TextBundle::from_section(" No ", TextStyle::default()).with_background_color(bg_no.into()));
                    bundles.push(TextBundle::from_section(" ", TextStyle::default()));
                    bundles.push(TextBundle::from_section(" Yes ", TextStyle::default()).with_background_color(bg_yes.into()));
                }
                let new_child = commands.spawn(NodeBundle::default())
